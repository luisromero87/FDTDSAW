...
def urollproc(procsx, procsy):
	return procsy*Nprocsx + procsx


Nx=int(find_parameter('Nx'))
Ny=int(find_parameter('Ny'))
Nz=int(find_parameter('Nz'))
Nprocsx=int(find_parameter('Nprocsx'))
Nprocsy=int(find_parameter('Nprocsy'))
...
for procsx in range(Nprocsx):
	for procsy in range(Nprocsy):
		    ...
		    for iy in range(PLM_w):
		        for iz in range(Nz):
					...
					if (procsy==0):
						...
						Omega_y[ix,PLM_w-1-iy,iz]=omega
					...
					if (procsy==Nprocsy-1):
						...
						Omega_y[ix,Ny-PLM_w+iy,iz]=omega
		...
		for ix in range(Nx):
		    for iy in range(Ny):
		        for iz in range(PLM_w):
		            ...
		            Omega_z[ix,iy,Nz-PLM_w+iz]=omega
		...
		f=open('PLM'+str(urollproc(procsx, procsy)).zfill(3),'wb') 
